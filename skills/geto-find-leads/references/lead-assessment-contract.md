...
- diligenceStatus 必须为 completed 或 completed_with_explicit_gaps。
- 评分对象是 CommercialAccount；Company 只承载统一主体。
- 客户价值模型代码为 `GETO_LEAD_VALUE`，并使用当前批准的 modelVersion。modelVersion 不可用时不计算总分。
- `capabilityFoundation.status` 必须为 available，并保存本次使用的 contentHash、productCodes、scenarioCodes、caseKeys 与 sourceKeys。partial/unavailable 时不计算正式总分。
...
- 与已知项目场景的相关性：8 分。
- 可执行的交易路径：4 分。
- 必须同时具备 GETO 能力底座映射和目标客户/项目侧 Claim/Source。没有客户场景证据时最高 8/20；没有能力底座时该维度保持未评分且不计算总分。共享工程量和互斥方案不得重复计分。
...
证据等级：A=权威直接证据，B=可信一手或多源一致，C=有限间接证据，U=无可用证据；对应权重 1、0.75、0.5、0。

`finalDimensionScore = observedScore × evidenceWeight + peerPriorScore × (1 - evidenceWeight)`。只有存在同市场、同客户类型且已冻结的 `cohortSnapshotKey` 时才允许使用 peerPrior；否则 U 级维度保持未评分，不得把 0 当事实。总分等于可发布的最终维度分之和，并保存 asOf、评分理由和逐维证据链。
