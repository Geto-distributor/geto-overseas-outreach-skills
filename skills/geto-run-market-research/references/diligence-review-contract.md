...
单公司任务的 final、callback、progress 区块和 validator 通过只表示任务已交付，不表示研究已经被国家主任务接受。国家主任务是独立审查者，必须以反证和追问为导向复核成果是否覆盖了可合理取得的公开信息、结论是否超过证据边界，以及任务是否在遇到困难后过早停止。

验收目标是“在声明的公开边界内充分穷尽”，不是声称看完互联网或要求取得私有资料。无法访问、平台分页受限、付费墙、登录墙、robots 限制和动态站点故障必须记录为边界；只有在边界、尝试路径、替代来源和剩余缺口均明确时，才可接受为有缺口的完成。
...
国家主任务读取 final 后必须同时打开 company.json、report.md、Sources/sources.md、关键来源页和 validator 输出，不照抄单公司任务的自证。对每家公司至少提出并回答以下挑战：
...
4. 官方社媒渠道是否被枚举？在默认最近 24 个月公开可见范围内，帖子是否逐页检查；更早但与产品发布、工厂、项目和管理层相关的里程碑是否补查？
...
国家主任务的审查不是措辞检查。即使报告写了所有栏目名，只要 Sources、researchQueries、项目事实或外部链接不能支持覆盖声明，仍应退回续查。
...
先从官网页脚、联系页、公开公司页和搜索结果归一官方 LinkedIn、Instagram、Facebook、YouTube、X 或当地主要平台。默认检查最近 24 个月内全部公开可见帖子；帖子量过大或平台限制完整分页时，必须：
...
- `accepted`：声明边界内的关键覆盖已穷尽，没有开放的 blocking 或 material challenge；
- `accepted_with_gaps`：关键路径已查，无法取得的信息有边界、替代来源、影响和下一步，没有开放的 blocking challenge；
- `returned_for_followup`：存在可通过继续公开检索或补读成果解决的实质缺口，必须把具体问题发回原任务；
- `rejected`：成果主体错误、证据污染、工件不可信，或无法作为该公司背调使用。

`accepted` 和 `accepted_with_gaps` 才能进入分类定稿、cohort 计算和导入准备。`returned_for_followup` 与 `rejected` 不得评分定稿或上传。
...
默认最多两次针对同一缺口的 follow-up。两次后若公开边界已经穷尽、没有新来源可用，主任务可接受为 `accepted_with_gaps`，但必须降低结论强度、优先级或信息完整度；若主体仍不稳定或 Evidence 仍污染，则使用 `rejected`/`identity_conflict`，不能为了结束任务而接受。
